    ...
    <div className="mx-auto max-w-[1100px] space-y-6 text-slate-100">
        ...
        <div>
          <div className="flex items-center gap-2">
            ...
            <span className="inline-flex items-center gap-1 rounded-full bg-gradient-to-r from-amber-500/20 to-indigo-500/20 border border-amber-500/30 px-3 py-1 text-[11px] font-extrabold uppercase text-amber-300 shadow-md">
              <Sparkles size={12} className="text-amber-400 animate-pulse" /> {t("sub.badge.soon", "Bientôt Disponible")}
            </span>
          </div>
      ...
      <div className="relative overflow-hidden rounded-3xl border border-indigo-500/20 bg-gradient-to-br from-slate-900 via-indigo-950/40 to-slate-950 p-8 sm:p-12 shadow-2xl">
        ...
        <div className="relative z-10 max-w-2xl space-y-5">
          ...
          <h2 className="font-display text-2xl sm:text-3xl font-black text-white tracking-tight leading-tight">
            {t("sub.hero.title", "Une nouvelle expérience d'abonnement arrive très bientôt sur")} <span className="bg-gradient-to-r from-indigo-400 via-purple-300 to-amber-300 bg-clip-text text-transparent">FATOURATI</span> !
          </h2>

          <p className="text-[14px] leading-relaxed text-slate-300">
            {t("sub.hero.desc", "Nous préparons des formules flexibles, transparentes et sur-mesure adaptées aux entrepreneurs, TPE et PME au Maroc. Profitez actuellement de toutes les fonctionnalités en version illimitée.")}
          </p>
          ...
          <div className="pt-2">
            {isSubscribed ? (
              <div className="inline-flex items-center gap-2 rounded-2xl bg-emerald-500/15 border border-emerald-500/30 px-5 py-3 text-[13px] font-bold text-emerald-300">
                ...
                <span>{t("sub.form.success", "Merci ! Vous serez notifié en priorité lors du lancement de nos offres.")}</span>
              </div>
            ) : (
              <form onSubmit={handleSubscribe} className="flex flex-col sm:flex-row gap-2.5 max-w-md">
                <div className="relative flex-1">
                  ...
                  <input
                    ...
                    placeholder={t("sub.form.placeholder", "Votre adresse email pro...")}
                ...
                >
                  {t("sub.form.submit", "M'avertir")} <ArrowRight size={14} />
                </button>
              </form>
            )}
          </div>
      ...
      <div>
        <p className="text-[12px] font-bold uppercase tracking-wider text-slate-400 mb-3 px-1">
          {t("sub.features.title", "Ce qui est en cours de préparation :")}
        </p>
        <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
          <div className="bento-card space-y-2">
            ...
            <h3 className="text-[14px] font-bold text-white">{t("sub.feat1.title", "IA Vision & Automatisation")}</h3>
            <p className="text-[12px] text-slate-400 leading-relaxed">
              {t("sub.feat1.desc", "Extraction automatique des devis & factures PDF par l'IA Gemini 2.5 et assistant bot WhatsApp.")}
            </p>
          </div>
          ...
          <div className="bento-card space-y-2">
            ...
            <h3 className="text-[14px] font-bold text-white">{t("sub.feat2.title", "Paiement CMI & Carte Marocaine")}</h3>
            <p className="text-[12px] text-slate-400 leading-relaxed">
              {t("sub.feat2.desc", "Intégration sécurisée des passerelles de paiement locales en Dirhams (MAD).")}
            </p>
          </div>
          ...
          <div className="bento-card space-y-2">
            ...
            <h3 className="text-[14px] font-bold text-white">{t("sub.feat3.title", "Accès Gratuit Actuel")}</h3>
            <p className="text-[12px] text-slate-400 leading-relaxed">
              {t("sub.feat3.desc", "Toutes les fonctionnalités restent 100% gratuites et illimitées pendant la phase de lancement.")}
            </p>
          </div>
        </div>
      </div>
