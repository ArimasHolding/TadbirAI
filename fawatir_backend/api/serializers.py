...
class ProductSerializer(TenantSerializerMixin):
    ...
    def get_quantity(self, obj):
        try:
            return sum(inv.quantity or 0 for inv in obj.inventory_records.all())
        except Exception:
            return 0
...
class InvoiceSerializer(TenantSerializerMixin):
    ...
    def get_client_name(self, obj):
        if obj.client:
            return obj.client.company_name or obj.client.contact_name or str(obj.client.id)
        return None
...
class DepenseSerializer(TenantSerializerMixin):
    total_amount = serializers.DecimalField(source='montant', max_digits=12, decimal_places=2, required=False)
    status = serializers.CharField(source='statut', required=False, allow_blank=True)
...
class AvoirSerializer(TenantSerializerMixin):
    class Meta:
        model = models.Avoir
        fields = '__all__'
        read_only_fields = ['organisation']

class BonCommandeSerializer(TenantSerializerMixin):
    class Meta:
        model = models.BonCommande
        fields = '__all__'
        read_only_fields = ['organisation']

class BulletinPaieSerializer(TenantSerializerMixin):
    class Meta:
        model = models.BulletinPaie
        fields = '__all__'
        read_only_fields = ['organisation']
...
class PurchaseOrderSerializer(TenantSerializerMixin):
    ...
    def get_supplier_name(self, obj):
        if obj.supplier:
            return obj.supplier.company_name or obj.supplier.contact_name or str(obj.supplier.id)
        return None
