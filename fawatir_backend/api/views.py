...
class OrganizationViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
    ...
    def get_queryset(self):
        qs = super(TenantIsolationMixin, self).get_queryset()
        user = getattr(self.request, 'user', None)
        if not user or not user.is_authenticated:
            return qs.none()
        
        # Tadbir tenant user logic
        user_email = getattr(user, 'email', None)
        tenant_user = None
        if user_email:
            tenant_user = models.User.objects.filter(email=user_email).first()
        ...
        if primary_org_id:
            org_ids.add(primary_org_id)
            
        # 2. Add owned organisations
        if tenant_user:
            owned_ids = models.Organization.objects.filter(owner=tenant_user).values_list('id', flat=True)
            org_ids.update([str(oid) for oid in owned_ids])
...
class OrganizationSettingViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
    ...
    def current(self, request):
        ...
        org = models.Organization.objects.filter(id=org_id).first()
        if not org:
            return Response({"error": "Organization not found"}, status=status.HTTP_404_NOT_FOUND)
            
        setting = models.OrganizationSetting.objects.filter(
            organisation=org
        ).order_by('created_at', 'id').first()
        if setting is None:
            setting = models.OrganizationSetting.objects.create(organisation=org)
        ...
        try:
            if request.method == 'GET':
                return Response({
                    ...
                    "adresse": org.address,
                    "telephone": org.phone,
                    "site_web": org.website,
                    "secteur": org.industry,
                    "pays": org.country,
                    "devise": org.currency,
                    "identifiant_fiscal": org.tax_identifier,
                    "ice": org.ice,
                    "registre_commerce": org.rc,
                    "tva_rate": str(setting.vat_rate) if setting.vat_rate else "",
                    "afficher_tva": setting.show_vat,
                    "montant_lettres": setting.amount_in_words,
                    "invoice_prefix": setting.invoice_prefix,
                    "quotation_prefix": setting.quotation_prefix,
                ...
                if currency:
                    currency = str(currency).split(' - ', 1)[0].strip().upper()
                    if len(currency) > 10:
                        return Response({"error": "Devise invalide"}, status=status.HTTP_400_BAD_REQUEST)

                with transaction.atomic():
                    # Update Organization
                    if 'nom' in data: org.name = data['nom']
                    if 'email' in data: org.email = data['email']
                    if 'adresse' in data: org.address = data['adresse']
                    if 'telephone' in data: org.phone = data['telephone']
                    if 'site_web' in data: org.website = data['site_web']
                    if 'secteur' in data: org.industry = data['secteur']
                    if 'pays' in data: org.country = data['pays']
                    if currency: org.currency = currency
                    if 'identifiant_fiscal' in data: org.tax_identifier = data['identifiant_fiscal']
                    if 'ice' in data: org.ice = data['ice']
                    if 'registre_commerce' in data: org.rc = data['registre_commerce']

                    # Additional fiscal fields for France
                    if 'siren' in data: org.tax_identifier = data['siren']
                    if 'siret' in data: org.ice = data['siret']
                    if 'tva_intra' in data: org.rc = data['tva_intra']

                    org.save()

                    # Update OrganizationSetting
                    if 'tva_rate' in data and data['tva_rate']: setting.vat_rate = data['tva_rate']
                    if 'afficher_tva' in data: setting.show_vat = data['afficher_tva']
                    if 'montant_lettres' in data: setting.amount_in_words = data['montant_lettres']
                    if 'invoice_prefix' in data: setting.invoice_prefix = data['invoice_prefix']
                    if 'quotation_prefix' in data: setting.quotation_prefix = data['quotation_prefix']
                    if 'purchase_prefix' in data: setting.purchase_prefix = data['purchase_prefix']
                    if 'iban' in data: setting.iban = data['iban']
                    if 'rib' in data: setting.rib = data['rib']
                    if 'swift' in data: setting.swift = data['swift']
                    if 'bank_name' in data: setting.bank_name = data['bank_name']
                    if 'bank_address' in data: setting.bank_address = data['bank_address']
                    if 'capital_social' in data: setting.capital_social = data['capital_social']
                    if 'cnss' in data: setting.cnss = data['cnss']
                    if 'footer_text' in data: setting.footer_text = data['footer_text']
                    if 'smtp_host' in data: setting.smtp_host = data['smtp_host']
                    if 'smtp_port' in data: setting.smtp_port = data['smtp_port']
                    if 'smtp_user' in data: setting.smtp_user = data['smtp_user']
                    if 'smtp_password' in data: setting.smtp_password = data['smtp_password']
...
class ProductViewSet(TenantIsolationMixin, viewsets.ModelViewSet):
    ...
    def _sync_product_extras(self, product, data):
        ...
        if isinstance(category_name, str) and category_name.strip().lower() not in ['général', 'general', '']:
        ...
        if 'sub_category' in data:
            ...
            metadata['sub_category'] = data.get('sub_category') or ''
            product.metadata = metadata
            changed = True

        if 'min_stock' in data or 'minimum_stock' in data:
            raw_minimum = data.get('min_stock', data.get('minimum_stock'))
            try:
                product.minimum_stock = max(0, int(float(str(raw_minimum).replace(',', '.'))))
        ...
        if 'quantity' in data:
            try:
                quantity = max(0, int(float(str(data.get('quantity') or 0).replace(',', '.'))))
            except (TypeError, ValueError):
                quantity = 0
            inventory_records = product.inventory_records.order_by('updated_at', 'id')
            inventory = inventory_records.first()
            if inventory is None:
                inventory = models.Inventory(product=product)
            else:
                inventory_records.exclude(id=inventory.id).delete()
            inventory.quantity = quantity
            inventory.available_quantity = quantity
            inventory.save()

    def perform_create(self, serializer):
        organisation_id = self.get_tenant_organisation_id()
        if not organisation_id:
            raise PermissionDenied("No organization found for this user.")
        organisation = models.Organization.objects.filter(id=organisation_id).first()
        if not organisation:
            raise PermissionDenied("Organization not found.")
        product = serializer.save(organisation=organisation)

        raw_quantity = self.request.data.get('quantity', 0)
        try:
            quantity = max(0, int(float(str(raw_quantity).replace(',', '.'))))
        except (TypeError, ValueError):
            quantity = 0

        if not product.inventory_records.exists():
            models.Inventory.objects.create(product=product, quantity=quantity, available_quantity=quantity)
        self._sync_product_extras(product, self.request.data)
    ...
    def create(self, request, *args, **kwargs):
        ...
        product = models.Product.objects.get(id=response.data['id'])

        changed = False

        # 1. Handle Category Name
        if category_name and isinstance(category_name, str) and category_name.strip().lower() not in ['général', 'general', '']:
        ...
        # Re-serialize to include computed fields
    ...
    def update(self, request, *args, **kwargs):
        ...
        self._sync_product_extras(product, request.data)
        return Response(self.get_serializer(product).data, status=response.status_code)
    ...
    def create(self, request, *args, **kwargs):
        if 'items' in request.data and isinstance(request.data['items'], list):
            ...
            for item in items:
                ...
                if not prod_id and item.get('create_if_missing'):
                    # Auto-create product
                    prod = models.Product.objects.create(
                        name=item.get('product_name', 'Unknown Product'),
                        sku=f"SKU-{uuid.uuid4().hex[:8].upper()}",
                        selling_price=item.get('selling_price', 0),
                        organisation=request.user.organisation if hasattr(request, 'user') and hasattr(request.user, 'organisation') else models.Organization.objects.first()
                    )
                    prod_id = prod.id
                ...
                if prod_id:
                    ...
                    try:
                        ...
                        mov = models.StockMovement.objects.create(
                            product_id=prod_id,
                            ...
                            reason=request.data.get('supplier', 'Auto Import'),
                            created_by=request.user if hasattr(request, 'user') and request.user.is_authenticated else None
                        )
                        created_movements.append(serializers.StockMovementSerializer(mov).data)
                    except Exception as e:
                        pass
            return Response({"status": "bulk_success", "created": len(created_movements), "data": created_movements})
